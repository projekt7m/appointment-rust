/*
 * Appointments Backend
 *
 * # API for appointment scheduling related data  This is the API of the service at P7M that manages the scheduling and management of appointments. It is used by the booking widget (see the **WidgetApi** tag) with functions that are public and don't require the user to be authenticated.  For endpoints in other tags the caller has to be authenticated with the system and provide a JWT token in the Authorization header of the HTTP request. When using the API you typically get this token by authenticating first with OAuth 2.0.  When you are trying this API using the Swagger interface, you need to click the `Authorize` button and then again the Authorize button in the pop-up that gets opened.
 *
 * The version of the OpenAPI document: 0.12.1
 * Contact: tech@p7m.de
 * Generated by: https://openapi-generator.tech
 */




#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct TenantConfig {
    #[serde(rename = "services")]
    pub services: Vec<crate::models::Service>,
    #[serde(rename = "workers")]
    pub workers: Vec<crate::models::WorkerConfig>,
    #[serde(rename = "name")]
    pub name: String,
    #[serde(rename = "address")]
    pub address: Vec<String>,
    #[serde(rename = "phone")]
    pub phone: String,
    #[serde(rename = "phone_national")]
    pub phone_national: String,
    #[serde(rename = "email")]
    pub email: String,
    #[serde(rename = "calendar_extra_info")]
    pub calendar_extra_info: String,
    #[serde(rename = "required_conditions")]
    pub required_conditions: Vec<String>,
    #[serde(rename = "time_zone")]
    pub time_zone: String,
}

impl TenantConfig {
    pub fn new(services: Vec<crate::models::Service>, workers: Vec<crate::models::WorkerConfig>, name: String, address: Vec<String>, phone: String, phone_national: String, email: String, calendar_extra_info: String, required_conditions: Vec<String>, time_zone: String) -> TenantConfig {
        TenantConfig {
            services,
            workers,
            name,
            address,
            phone,
            phone_national,
            email,
            calendar_extra_info,
            required_conditions,
            time_zone,
        }
    }
}


